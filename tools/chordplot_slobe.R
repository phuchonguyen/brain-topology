...
# Level 3 
chord_sublobe <- function(ribcol="blue", gap.after=1, labeled=TRUE,
                          h.ratio=0.5, A=NULL, corcol=NULL, track.col="lightgray") {
  midid <- (length(L4)/2)
  factors <- c(L4[1:midid], "blank1",
               L4[(midid + 1):length(L4)], "blank2")
  ribsize <- SUBLOBE_RIBSIZE(gap.after = gap.after)
  xlim <- rbind(cbind(rep(0, midid), ribsize[1:midid]), 
                c(0,10),
                cbind(rep(0, midid), ribsize[(midid+1):length(L4)]),
                c(0,10))
  ...
  if (!Agiven) {
    ...
    Amax <- max(A)
    Amin <- min(A)
    A <- get_chord_mat(NODES, A, l3id, fun="mean")
